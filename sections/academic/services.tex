% \skills{Cinnamon AI Bootcamp 2023}\\
% \subtext{Mentoring}
% \begin{zitemize}
%     \item Mentored a group of 4, which won the 3rd prize for their graduation project.
%     \item 2 students received internship offers (out of 5 total offers).
%     \item (These kids gave me serious peer-pressure).
%     \item Design syllabus, prepared entrance tests.
% \end{zitemize}

% \skills{Cinnamon AI Bootcamp 2022}\\
% \subtext{Mentoring}
% \begin{zitemize}
%     \item Mentored a group of 4, which won the 2nd prize for their graduation project.
%     \item (These kids are amazing, far more gifted than I could ever be).
%     \item Design syllabus, prepared entrance tests, interviewed candidates.
% \end{zitemize}

% \skills{Cinnamon AI Bootcamp 2020}\\
% \subtext{Teaching/Mentoring}
% \begin{zitemize}
%     \item Mentored a group of 3.
%     \item Gave lectures on Language Modelling and Transformers to a class of 15 students, most of them hold a Master degree or are Master students.
%     \item Contributed to syllabus design, interviewed candidates.
% \end{zitemize}


\skills{Reviewing}\\
\subtext{ICML 2026, ICLR 2026, MoFA @ ICML 2025, XLLM-Reason-Plan @ COLM 2025}
% \vspace{1em}

\skills{Teaching/Mentoring}\\
\subtext{Cinnamon AI Bootcamp 2020, 2022, 2023}
\begin{zitemize}
    \item Mentored groups of 3-4 students.
    \item Designed syllabus, prepared entrance tests, interviewed candidates.
    \item Prepared materials and gave lectures on Language Modelling and Transformers.
\end{zitemize}

\skills{Conference/Workshop Presentations}
\begin{zitemize}
    \item Angular Steering: Behavior Control via Rotation in Activation Space. \textit{NeurIPS 2025}
    (Spotlight Poster); \textit{Mechanistic Interpretability Workshop at NeurIPS 2025} (Poster).
    \item A Span Extraction Approach for Information Extraction on Visually-Rich Documents.
    \textit{ICDAR 2021, Workshop on Document Images and Language} (Oral).
\end{zitemize}